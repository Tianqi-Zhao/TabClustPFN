...
class ColEmbedding(nn.Module):
    ...
    def _compute_embeddings(self, features: Tensor) -> Tensor:
    ...
    def _train_forward(self, X: Tensor, d: Optional[Tensor] = None) -> Tensor:
        ...
        if d is None:
            ...
            embeddings = self._compute_embeddings(features)  # (B, H+C, T, E)
        else:
            if self.reserve_cls_tokens > 0:
            ...
            effective_embeddings = self._compute_embeddings(features)  # (N, T, E)
    ...
    def _inference_forward(
        ...
        if feature_shuffles is None:
            # Can be used when freeze parameters
            ...
            embeddings = self.inference_mgr(
                self._compute_embeddings, inputs=OrderedDict([("features", features)])
            )  # (B, H+C, T, E)
        else:
            B = X.shape[0]
            ...
            first_embeddings = self.inference_mgr(
                ...
                inputs=OrderedDict([("features", features)]),
                output_repeat=B,
            )  # (H+C, T, E)
    ...
    def forward(
        ...
        if self.training:
            embeddings = self._train_forward(X, d)
        else:
            embeddings = self._inference_forward(X, feature_shuffles, mgr_config)
