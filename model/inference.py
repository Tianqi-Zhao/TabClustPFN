...
class MemoryEstimator:
    ...
    coefficients: Dict[str, list] = {
        ...
        "tf_dataset": [-2.60068961e-01, 4.77470594e-07, 1.95310976e-02],
    }
    intercepts: Dict[str, float] = {
        "tf_col": 137.62474190864668,
        "tf_row": 138.53653545318957,
        "tf_dataset": 140.58027172987750,
    }
    ...
    def estimate_peak_mem(
        ...
        enc_name : str
            ...
            - "tf_dataset": Cluster learning encoder
...
class InferenceManager:
    ...
    out_no_seq : bool or List[bool], default=False
        Whether to remove sequence dimension from output tensor(s). 
        - If bool: applies to all outputs (or single output if out_dims is None)
        - If List[bool]: each element specifies whether the corresponding output has no seq dimension
          For example, [False, True] means first output has shape (..., seq_len, dim1), 
          second output has shape (..., dim2) without seq_len
    
    out_dims : Optional[List[int | Tuple[int, ...]]], default=None
        For multi-output support. If provided, the forward function is expected to return
        a tuple of tensors with dimensions specified in this list. Each element can be:
        - int: represents a single trailing dimension, e.g., 100 means shape (..., 100)
        - tuple: represents multiple trailing dimensions, e.g., (100, 2) means shape (..., 100, 2)
        
        Examples:
        - out_dims=[100, 200] means outputs have shapes (..., 100) and (..., 200)
        - out_dims=[100, (50, 2)] means outputs have shapes (..., 100) and (..., 50, 2)
    """

    def __init__(self, enc_name: str, out_dim: int, out_no_seq: bool | List[bool] = False, out_dims: Optional[List[int | Tuple[int, ...]]] = None):
        self.enc_name = enc_name
        self.out_dim = out_dim
        
        # Normalize out_dims to list of tuples for consistency
        if out_dims is not None:
            self.out_dims_normalized = []
            for dim in out_dims:
                if isinstance(dim, int):
                    self.out_dims_normalized.append((dim,))
                else:
                    self.out_dims_normalized.append(tuple(dim))
        else:
            self.out_dims_normalized = None
        
        # Handle out_no_seq as either bool or list
        if out_dims is not None:
            # Multi-output mode
            if isinstance(out_no_seq, bool):
                # If bool, apply to all outputs
                self.out_no_seq_list = [out_no_seq] * len(out_dims)
            else:
                # If list, use as-is
                self.out_no_seq_list = out_no_seq
                assert len(self.out_no_seq_list) == len(out_dims), \
                    f"out_no_seq list length ({len(self.out_no_seq_list)}) must match out_dims length ({len(out_dims)})"
        else:
            # Single output mode
            self.out_no_seq_list = [out_no_seq] if isinstance(out_no_seq, bool) else out_no_seq
        
        self.out_no_seq = out_no_seq  # Keep for backward compatibility
        self.out_dims = out_dims  # For multi-output support: list of output dimensions
    ...
    def __call__(
        self,
        forward_fn: Callable[..., Tensor | Tuple[Tensor, ...]],
        ...
        ----------
        forward_fn : Callable[..., Tensor | Tuple[Tensor, ...]]
            Model forward function that takes dictionary inputs and returns tensor output(s).
            If the function returns multiple outputs (a tuple), you must specify out_dims
            when creating InferenceManager to enable multi-output support.
        ...
        Tensor or Tuple[Tensor, ...]
            Combined output from all batches. If out_dims is specified and forward_fn returns
            a tuple, returns a tuple of tensors. Otherwise returns a single tensor.
            
        Raises
        ------
        ValueError
            If forward_fn returns a tuple but out_dims is None, or if the number of outputs
            doesn't match the number of out_dims specified.
        ...
        if not auto_batch:
            ...
            if self.offload:
                if self.out_dims is not None and isinstance(outputs, tuple):
                    # Multi-output mode: move all tensors to CPU
                    return tuple(out.to(device="cpu") for out in outputs)
                else:
        ...
        *batch_dims, seq_len, in_dim = first_value.shape
        ...
        # Define output shape(s) based on configuration
        if self.out_dims is not None:
            # Multi-output mode: create multiple output shapes
            output_shapes = []
            for i, out_dims_tuple in enumerate(self.out_dims_normalized):
                if self.out_no_seq_list[i]:
                    # No sequence dimension for this output: (batch_dims, *out_dims_tuple)
                    output_shapes.append((*batch_dims, *out_dims_tuple))
                else:
                    # Has sequence dimension for this output: (batch_dims, seq_len, *out_dims_tuple)
                    output_shapes.append((*batch_dims, seq_len, *out_dims_tuple))
        else:
            # Single output mode
            if self.out_no_seq_list[0]:
        ...
        if self.offload == "auto":
            ...
            if self.out_dims is not None:
                # Multi-output: sum all output sizes
                total_output_mb = sum(bytes_per_element * math.prod(shape) / (1024 * 1024) for shape in output_shapes)
            else:
                # Single output
                total_output_mb = bytes_per_element * math.prod(output_shape) / (1024 * 1024)
            
            total_output_mb *= output_repeat  # Multiply by repeat factor

            # Check if output size exceeds threshold of available GPU memory
            output_pct = total_output_mb / gpu_mem
            ...
            enough_cpu = cpu_mem > total_output_mb

            self.offload = excess_gpu and enough_cpu
            if self.verbose:
                print(
                    f"Output size: {total_output_mb / 1024:.2f}GB, "
        ...
        # Pre-allocate output tensor(s) with same dtype as input
        output_device = torch.device("cpu") if self.offload else self.exe_device
        
        if self.out_dims is not None:
            # Multi-output mode: pre-allocate multiple tensors
            outputs = tuple(
                torch.empty(shape, dtype=input_dtype, device=output_device)
            ...
            try:
                ...
                for batch_dict, indices in batch_iterator:
                    with torch.no_grad():
                        ...
                        if self.out_dims is not None:
                            # Multi-output mode: handle tuple of outputs
                            if not isinstance(output, tuple):
                                raise ValueError(f"Expected tuple output when out_dims is specified, got {type(output)}")
                            
                            if len(output) != len(self.out_dims_normalized):
                                raise ValueError(
                                    f"Number of outputs ({len(output)}) does not match number of out_dims "
                                    f"({len(self.out_dims_normalized)}). Expected {len(self.out_dims_normalized)} outputs."
                                )
                            
                            for i, out_tensor in enumerate(output):
                                # Note: indices only contains batch dimension slices (from slice_tuple)
                                # For outputs with seq dimension, PyTorch will automatically handle it
                                # For outputs without seq dimension, indices directly maps to the output shape
                                if self.offload:
                                    outputs[i][indices] = out_tensor.to(device="cpu")
                                else:
                                    outputs[i][indices] = out_tensor
                            del output
                        else:
                            # Single output mode
                            if isinstance(output, tuple):
                                raise ValueError(
                                    f"Forward function returned a tuple with {len(output)} outputs, but out_dims is None. "
                                    f"To handle multiple outputs, please specify out_dims parameter when creating "
                                    f"InferenceManager. For example: out_dims=[dim1, dim2, ...]"
                                )
                            if self.offload:
